...
class User
  ...
  class << self
    ...
    def setup_prepared_statements
      ...
      Database.connection.prepare('user_insert', 'INSERT INTO users (first_name, last_name, email, telephone, password_hash) 
      VALUES($1, $2, $3, $4, $5);')
    end
    ...
    def create_user(user, password)
      Database.connection.exec_prepared('user_insert', [user.first_name, user.last_name, user.email, user.telephone, BCrypt::Password.create(password)])
    end
  end
end